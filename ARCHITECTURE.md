...
| Does the overnight rule survive **walk-forward**? | **Yes, with a measured overfitting tax.** [`walk_forward_overnight.py`](walk_forward_overnight.py): 4-year train / 1-year test, rolling origin, **16 non-overlapping out-of-sample windows** (2011-2026). Parameters chosen using only the training window each time. **OOS: 31.8% CAGR, Sharpe 1.56, maxDD -30%** at zero cost, against SPY's 14.4%/0.88 and EW-hold-all 13.4%/0.79. In-sample optimum was 46.5%, so the **overfitting gap is ~14 points of CAGR and ~0.3 of Sharpe, and it is stable across every cost level.** Parameter choice is stable too (lb=252/N=10 in 9 of 16 windows, lb=126/N=10 in 6). Unlike the undifferentiated version, it clears every benchmark at 1bp/side (25.3% vs SPY 14.4%). Bad years exist: 2022 OOS was -5.1%. |
| Is the overnight signal just a **volatility tilt**? | **Mostly, but not entirely — and this is the biggest caveat on the result.** Out-of-sample at N=33, zero cost: random selection **9.3%** (the floor), volatility-ranked **17.2%**, overnight-signal-ranked **21.9%**. So volatility explains roughly **63%** of the edge over random and the overnight signal adds the remaining **37%**. The proportions hold at every cost level. The signal is real and separable, but anyone describing this as a pure overnight anomaly is overstating it: most of the lift is from tilting into high-volatility names. |
| Is the edge the timing or the selection? (out-of-sample) | **Timing, confirmed OOS.** The identical walk-forward basket held ALL DAY returns 21.5% at Sharpe 0.82; held overnight only it returns 31.8% at 1.56. |
| Does concentration break it? | **Less than feared, but the test has a blind spot.** [`stress_gaps_overnight.py`](stress_gaps_overnight.py), same hazard model as `stress_gaps.py`: worst-case drawdown is **-37% at N=10 against -32% at N=83** — a far narrower spread than Strategy C's -77%/-31%, because a 10% position can absorb a -90% single-name gap. **But the Monte Carlo injects INDEPENDENT single-name gaps and does not model a correlated market-wide overnight gap, which is this strategy's dominant tail risk.** It is 100% exposed to exactly that window and earns nothing intraday to cushion it. |
